      ...
      <template #item="{ element, index }">
        <div class="tw-group tw-flex tw-items-center tw-justify-between tw-bg-white tw-p-4 tw-rounded-2xl tw-border tw-border-slate-200 tw-shadow-sm hover:tw-border-indigo-200 tw-transition-all">
          ...
          <div class="tw-flex tw-gap-2">
            ...
            <q-btn @click.stop="$emit('delete-filter', index)" icon="fa-light fa-trash-can" color="red-4" flat round dense class="tw-opacity-0 group-hover:tw-opacity-100 tw-transition-opacity" />
          </div>
        </div>
      </template>
    </draggable>
