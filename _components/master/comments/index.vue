<template>
  <div
    ...
    class="tw-max-w-4xl tw-mx-auto tw-antialiased"
  >
    <div>
      <q-list dense class="list-comments">
        ...
        >
          <q-item-section top avatar>
            <q-avatar
              size="42px"
              class="tw-shadow-sm tw-border-2 tw-border-white"
            >
            ...
            >
              ...
              >
                ...
                <q-icon
                  ...
                  size="xs"
                  class="group-hover:tw-text-primary tw-opacity-0 group-hover:tw-opacity-100 tw-transition-all"
                />
              </q-card-section>
            </q-card>
          </q-item-section>
          ...
          <q-item-section
            class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-border tw-border-gray-100 tw-p-4"
            v-else
          >
        ...
        <q-item v-if="permisionComments.index" class="tw-p-0">
          <q-item-section v-if="!loading">
            <q-timeline color="primary" class="tw-px-2">
              ...
              >
                <div
                  class="tw-group tw-relative tw-p-4 tw-rounded-2xl tw-bg-white tw-border tw-border-gray-100 tw-shadow-sm hover:tw-shadow-md tw-transition-all tw-duration-300"
                >
                  <div>
                    <div
                    ...
                    >
                      <h4
                        class="tw-text-sm tw-m-0 tw-flex tw-items-center tw-gap-2"
                      >
                        <span
                          v-if="
                            item.userProfile &&
                            !Boolean(item.internal || item.isInternal)
                          "
                          class="tw-font-bold tw-text-gray-800"
                        >
                    ...
                    <div v-else>
                      <div
                        class="tw-relative"
                        :class="{
                          'tw-cursor-pointer group/content': !Boolean(
                            item.internal || item.isInternal
                          ),
                        }"
                        @click="
                          !Boolean(item.internal || item.isInternal)
                      ...
                      <div
                        class="tw-mt-4"
                        v-if="item.files.length > 0 && permissionMedia.index"
                      >
                        <div
                          class="tw-flex tw-items-center tw-gap-2 tw-mb-3 tw-text-gray-500 tw-font-bold tw-text-xs tw-uppercase tw-tracking-wider"
                        >
                        ...
                        >
                          ...
                          >
                            <div
                              class="tw-w-8 tw-h-8 tw-flex-shrink-0 tw-rounded-lg tw-flex tw-items-center tw-justify-center tw-cursor-pointer tw-bg-gray-50"
                              @click="openPreview(file)"
                            >
                              <i
                                :class="[getFileIcon(file.name), 'tw-text-xl ']"
                              />
                            </div>

                            <div
                              class="tw-flex-1 tw-min-w-0 tw-flex tw-items-center tw-justify-between"
                            >
                              <div class="tw-truncate">
                                <template v-if="isPreviewable(file)">
                                  <span
                                    @click="openPreview(file)"
                                    class="tw-block tw-truncate tw-text-[13px] tw-font-bold tw-text-gray-700 tw-no-underline hover:tw-text-primary tw-cursor-pointer"
                                  >
                    ...
                    <div
                      v-if="item.active"
                      class="tw-mt-4 tw-flex tw-items-start tw-justify-between"
                    >
                      <div class="tw-flex tw-items-start tw-space-x-3">
                        <q-btn
          ...
          <div
            v-if="loading"
            class="tw-w-full tw-flex tw-justify-center tw-py-12"
          >
      ...
      <div
        v-if="!loading && permisionComments.index && total > perPage"
  ...
  <filePreviewModal v-model="showPreview" :file="activeFile" />
</template>
